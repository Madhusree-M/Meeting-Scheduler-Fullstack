...
public class MeetingSlotController {
    ...
    // View all slots for a specific meeting
    @GetMapping("/meeting/{meetingId}")
    public String getSlotsByMeeting(@PathVariable Long meetingId, Model model) {
        Meeting meeting = meetingService.getMeetingById(meetingId)
                .orElseThrow(() -> new RuntimeException("Meeting not found with ID: " + meetingId));
        List<MeetingSlot> slots = slotService.getSlotsByMeeting(meeting);
        model.addAttribute("meeting", meeting);
        model.addAttribute("slots", slots);
        return "slots/list-by-meeting"; // new Thymeleaf template
    }

    // View single slot details
    @GetMapping("/{id}/view")
    public String viewSlot(@PathVariable Long id, Model model) {
        MeetingSlot slot = slotService.getSlotById(id)
                .orElseThrow(() -> new RuntimeException("Slot not found with ID : " + id));
        model.addAttribute("slot", slot);
        return "slots/view"; // new Thymeleaf template
    }
}
...
// return slotService.getSlotById(id).orElseThrow(() -> new
// RuntimeException("Slot not found with Id : " + id));
// }
...
// @PutMapping("/{id}")
// public MeetingSlot updateSlot(@PathVariable Long id, @RequestBody MeetingSlot
// updatedSlot) {
// MeetingSlot existingSlot = slotService.getSlotById(id)
// .orElseThrow(() -> new RuntimeException("Slot not found with ID: " + id));

// if (updatedSlot.getMeeting() != null &&
// updatedSlot.getMeeting().getMeetingId() != null) {
// Meeting meeting =
// meetingService.getMeetingById(updatedSlot.getMeeting().getMeetingId())
