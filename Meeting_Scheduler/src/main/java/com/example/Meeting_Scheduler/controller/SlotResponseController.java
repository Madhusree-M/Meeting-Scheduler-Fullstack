...
public class SlotResponseController {
    ...
    // see all responses for a specific slot
    @GetMapping("/slot/{slotId}")
    public String getResponsesBySlot(@PathVariable Long slotId, Model model) {
        MeetingSlot slot = slotService.getSlotById(slotId)
                .orElseThrow(() -> new RuntimeException("Slot not found with Id : " + slotId));
        model.addAttribute("slot", slot);
        model.addAttribute("responses", responseService.getResponsesBySlot(slot));
        return "responses/list-by-slot"; // new template
    }
    ...
    public String viewResponse(@PathVariable Long id, Model model) {
        SlotResponse response = responseService.getResponseById(id)
                .orElseThrow(() -> new RuntimeException("Response not found with Id : " + id));
        model.addAttribute("response", response);
        return "responses/view"; // Thymeleaf template to show details
    }
...
// .orElseThrow(() -> new RuntimeException("Response not found with Id : " +
// id));
// }
...
// User participant =
// userService.getUserById(response.getParticipant().getUserId()).orElseThrow(
// () -> new RuntimeException("User not found with Id : " +
// response.getParticipant().getUserId()));
...
// // Update existing response
// @PutMapping("/{id}")
// public SlotResponse updateResponse(@PathVariable Long id, @RequestBody
// SlotResponse updatedResponse) {
// SlotResponse existingResponse = responseService.getResponseById(id)
// .orElseThrow(() -> new RuntimeException("Response not found with Id : " +
// id));

// // base condition -> Updating slot if exists
// if (updatedResponse.getSlot() != null &&
// updatedResponse.getSlot().getSlotId() != null) {
// MeetingSlot slot =
// slotService.getSlotById(updatedResponse.getSlot().getSlotId()).orElseThrow(
// () -> new RuntimeException("Slot not found with Id : " +
// updatedResponse.getSlot().getSlotId()));
// existingResponse.setSlot(slot);
// }

// // base condition -> Updating participant if exists
// if (updatedResponse.getParticipant() != null &&
// updatedResponse.getParticipant().getUserId() != null) {
// User participant =
// userService.getUserById(updatedResponse.getParticipant().getUserId())
// .orElseThrow(() -> new RuntimeException(
// "Participant not found with Id : " +
// updatedResponse.getParticipant().getUserId()));
// existingResponse.setParticipant(participant);
// }
